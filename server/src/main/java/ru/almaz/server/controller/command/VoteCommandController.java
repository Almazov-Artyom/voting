...
public class VoteCommandController implements CommandController {
    @Value("${client.template.command.vote}")
    private String voteTemplate;

    private final VoteService voteService;

    @Override
    public void accept(ChannelHandlerContext ctx, String message) {
        voteService.startVote(ctx, message);
    }
